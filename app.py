...
def main():
    ...
    with st.sidebar:
        ...
        if uploaded_file:
                 ...
                 try:
                    with st.spinner("Analyzing..."):
                        ...
                        if api_key:
                            ...
                            # Robust fallback
                            model = None
                            for m in ['gemini-1.5-flash', 'gemini-1.5-flash-001', 'gemini-pro-vision']:
                                try:
            ...
            # Note: We need variables from main area (canvas) but button is in sidebar.
            # Streamlit logic: Input widgets in sidebar update session state.
            # Processing trigger logic is here.
            
            # We use Session State to trigger processing in Main area to ensure flow?
    ...
    if uploaded_file:
        ...
        # Calculate max width for canvas based on screen (heuristic)
        # In a real Map app, it fills the screen.
        
        st.markdown("### Canvas Area")
        st.caption("Define the target area for Price Crafting.")
        
        # Canvas Logic
        # 1. We keep canvas full width of the main column (which is remaining width)
        
        # We need a dynamic key (file_id)
        # We also need to READ the canvas state.
        
        # Issue: If button is in sidebar, how do we get canvas data?
        # st_canvas returns an object. It updates on interaction.
        # When 'Generate' in sidebar is clicked, the script reruns.
        # We can access the canvas object again.
        ...
        canvas_result = st_canvas(
            ...
            height=int(h * (800/w)) if w > 800 else h, # Basic scaling
            width=800 if w > 800 else w,
        ...
        if st.session_state.get('trigger_process', False):
            if canvas_result.json_data is not None and len(canvas_result.json_data["objects"]) > 0:
                 ...
                 points = []
                 # Scaling logic
                 scale_fac = 800/w if w > 800 else 1.0
                 
                 for p in path:
                    if p[0] == 'M' or p[0] == 'L':
                        points.append({'x': p[1] / scale_fac, 'y': p[2] / scale_fac})
                 ...
                 if len(points) >= 3:
                     with st.spinner("Crafting Pixels..."):
                         ...
                         # Rerun removed to avoid clearing state abruptly, instead we show result below
                         # st.rerun() 
            else:
